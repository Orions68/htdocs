...
if (isset($_POST["invited"])) // Si la cita la reserva el administrador.
{
    ...
    $stmt = $conn->prepare("SELECT id, email FROM clients"); // Busco los E-mail.
    $stmt->execute();
    if ($stmt->rowCount() > 0)
    {
...
$title = "Citas a Clientes - Salón Joana";
                ...
                <br><br><br><br>
                    ...
                    if (isset($_SESSION["client"]))
                    {
                        ...
                        else
                        {
                            ...
                            $z = 0;
                            $time_array = [];
                            $stmt = $conn->prepare("SELECT time FROM clients WHERE date='$date'");
                            $stmt->execute();
                            if ($stmt->rowCount() > 0)
                            {
                                while ($row = $stmt->fetch(PDO::FETCH_OBJ))
