<?php
        ...
        foreach ($files as $name => $file) // Para cada nombre $name de archivo en la lista de archivos
        {
            ...
            {
                $relativePath = substr($file, strlen($rootPath) + 1); // Asigna a $relativePath solo el nombre del archivo, a la ruta completa al archivo $file, le resta la ruta del directorio más la \.
                $zip->addFile($file, $relativePath); // Agrega los Archivos a backup.zip, $file es la ruta completa al archivo, $relativePath nombre del archivo que estará en el archivo backup.zip.
            }
        }
