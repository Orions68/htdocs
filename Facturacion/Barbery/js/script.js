...
function change(page, qtty, where) // Función que muestra los resultados de a 6 en filas y columnas de bootstrap, recibe la pagina page y la cantidad de resultados a mostrar qtty.
{
    ...
    if (length < 4) // Si la cantidad de artículos es menor que 4.
    {
        ...
        {
            var html = "<table><tr><th>Servicio</th><th>Precio</th><th>Cantidad</th><th>Parcial</th><th>I.V.A.</th><th>Total</th><th>Fecha</th><th>Hora</th></tr>"; // Muestro una tabla con las facturas del cliente.
            for (i = 0; i < qtty; i++) // Hago un bucle hasta la cantidad de resultados a mostrar.
            {
                if (i < length) // Si i es menor que el tamaño del array.
                {
                    ...
                    var my_qtty = qtties[i].split(",");
                    var final_qtty = "";
                    for (j = 0; j < my_qtty.length; j++)
                    {
                        final_qtty += my_qtty[j] + "<br>";
                    }
                    html += "<tr><td>" + service[i] + "</td><td>" + price[i] + "</td><td>" + final_qtty + "</td><td>" + total[i] * 100 / 121 + " $</td><td>" + (total[i] * 100 / 121) * .21 + " $</td><td>" + total[i] + "</td><td>" + my_date[2] + "/" + my_date[1] + "/" + my_date[0] + "</td><td>" + time[i] + "</td></tr>";
                }
    ...
    else // Si hay más de 3 resultados.
    {
        if (where == "profile") // Si se llama desde el perfil del cliente.
        {
            var html = "<table><tr><th>Servicio</th><th>Precio</th><th>Cantidad</th><th>Parcial</th><th>I.V.A.</th><th>Total</th><th>Fecha</th><th>Hora</th></tr>";
            for (i = (page - 1) * qtty; i < qtty + ((page - 1) * qtty); i++) // Aquí hago el bucle desde la página donde esté, a la cantidad de resultados a mostrar.
            {
                if (i < length) // Si i es menor que el tamaño del array.
                {
                    ...
                    html += "<tr><td>" + service[i] + "</td><td style='text-align: right;'>" + price[i] + "</td><td style='text-align: right;'><br>" + final_qtty + "</td><td>" + total[i] * 100 / 121 + " $</td><td>" + (total[i] * 100 / 121) * .21 + " $</td><td>" + total[i] + " $</td><td>" + my_date[2] + "/" + my_date[1] + "/" + my_date[0] + "</td><td>" + time[i] + "</td></tr>";
                }
                else
                {
                    break;
