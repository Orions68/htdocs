...
$ok = false;
    if (isset($_POST["email"])) // Verifico si llegan datos por post.
    {
        ...
        $ok = show($conn, $sql, $pass, "comp"); // Asigno el resultado de la función show() a la variable $ok, a show le paso la conexion, la consulta y la Contraseña.
    }
    else // Si no hay datos por post.
    {
        if (!empty($_SESSION["id"])) // Verifico que la sesión esté iniciada.
        {
