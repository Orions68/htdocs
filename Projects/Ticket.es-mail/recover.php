...
if (isset($_POST["email"]))
{
    ...
    $ok = false;
    $sql = "SELECT email FROM clients;"; // Preparo una consulta por la ID.
    $stmt = $conn->prepare($sql);
    $stmt->execute();
    while($row = $stmt->fetch(PDO::FETCH_OBJ)) // Asigno a la variable $row el contenido asociado a $resultado.
    {
        if ($row->email == $email)
        {
            $ok = true;
    ...
    if (!$ok)
    {
        echo "<script>toast(2, 'Hay un Error', 'Lo Siento no Existe Ningún Espectador con E-mail:  $email, Vuelve a Intentarlo con la Dirección que te Registrate.');</script>";
    }
    else
    {
		...
		{
			echo "<script>toast(0, 'Todo ha Ido Bien', 'Te Hemos Enviado una Contraseña Provisoria a: $email, Vuelve a Iniciar Sesión con los Nuevos Datos y Modifica la Contraseña, Gracias por Comprar tus Entradas Aquí.');</script>";
		}
		else
		{
            ...
            <div class="col-md-10">
                ...
                <br><br><br><br>
                    ...
                    <form action="" method="post">
                        <label><input type="email" name="email"> Danos el E-mail con el que te Registraste</label>
                        <br><br>
