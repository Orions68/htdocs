...
{
    $id = $_SESSION["id"]; // Cargo los datos del formulario en variable.
    $kind = $_POST["kind"];
    $place = $_POST["place"];
    $price = $_POST["price"];
    $start = $_POST["start"];
    ...
    {
        $path = "companies/emp-" . $_SESSION['id'] . "/" . $id; // Se asigna a $path la ruta donde se guardarán las imágenes del evento a publicar.
    ...
    switch (count($my_path)) // Tengo que insertar los datos según la cantidad de archivos subidos.
    {
        case 3: // Si se subieron 3 fotos.
            $stmt->execute(array(':id' => null, ':id_event' => $id, ':kind' => $kind, ':title' => $event, ':description' => $desc, ':path' => $my_path[0] . "·" . $my_path[1] . "·" . $my_path[2])); // La Ejecuto.
            break; // Ejecuto el Insert con las 3 rutas a las imágenes concatenadas y separadas por el *.
        case 2: // Si se subieron 2 fotos.
            $stmt->execute(array(':id' => null, ':id_event' => $id, ':kind' => $kind, ':title' => $event, ':description' => $desc, ':path' => $my_path[0] . "·" . $my_path[1])); // La Ejecuto.
            break; // Ejecuto el Insert con las 2 rutas a las imágenes concatenadas y separadas por el *.
        default: // Si se subió 1 fotos.
            $stmt->execute(array(':id' => null, ':id_event' => $id, ':kind' => $kind, ':title' => $event, ':description' => $desc, ':path' => $my_path[0])); // La Ejecuto.
